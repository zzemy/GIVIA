...
exports.buildCulturalAnalysisMessages = buildCulturalAnalysisMessages;
exports.buildRiskEnhancementPrompt = buildRiskEnhancementPrompt;
var shared_1 = require("@/lib/ai/prompts/shared");
function getOutputLanguageConstraint(language, countryName) {
    if (language === 'en') {
        return "All text must be in English. EXCEPTION: The 'card' (Greeting Card) content MUST be written in the native language spoken in ".concat(countryName, " (e.g. if country is Japan, write the card in Japanese).");
    }
    return "\u9664 'card' (\u8D3A\u5361) \u4E4B\u5916\u7684\u6240\u6709\u6587\u6848\u5FC5\u987B\u4F7F\u7528\u7B80\u4F53\u4E2D\u6587\u3002\u4F8B\u5916\uFF1A'card' \u5185\u90E8\u7684\u6587\u6848\u5FC5\u987B\u4F7F\u7528 ".concat(countryName, " \u5F53\u5730\u7684\u6BCD\u8BED\uFF08\u4F8B\u5982\uFF1A\u5982\u679C\u662F\u65E5\u672C\u5219\u5FC5\u987B\u7528\u65E5\u8BED\u5199\u8D3A\u5361\uFF0C\u6CD5\u56FD\u7528\u6CD5\u8BED\uFF0C\u82F1\u56FD\u7528\u82F1\u8BED\u7B49\uFF09\u3002");
}
function buildCulturalAnalysisMessages(input) {
    var language = input.language, country = input.country, recognition = input.recognition, giftContext = input.giftContext, audience = input.audience;
    return (0, shared_1.buildPromptMessages)({
        sections: {
...
function buildRiskEnhancementPrompt(input) {
    var isZh = input.locale === 'zh';
    var ruleExplanation = input.ruleResult.matchedRules
        .slice(0, 3)
        .map(function (rule) { return rule.explanation; })
        .join('; ');
    return (0, shared_1.renderPromptSections)({
        roleMission: [
            isZh
